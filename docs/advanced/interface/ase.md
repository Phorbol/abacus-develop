...
For socket-driven ASE workflows, use the `AbacusSocketIO` calculator. ASE runs the i-PI socket server, while ABACUS keeps `calculation=scf` and is launched with `socket_driver=1` as the client. Energy, forces, and stress are independent calculator properties. The i-PI `GETFORCE` wire layout is fixed, so an absent force or virial field is sent as zero padding together with an extras metadata record; `AbacusSocketIO` never exposes padding as a computed property. See the [ASE socket I/O documentation](https://ase-lib.org/ase/calculators/socketio/socketio.html) and the i-PI reference paper, [Ceriotti et al., Comput. Phys. Commun. 185, 1019-1026 (2014)](https://doi.org/10.1016/j.cpc.2013.10.027), for the protocol background.
...
`AbacusSocketIO` sets `socket_driver=1` and selects the socket endpoint, passing it to ABACUS through `ABACUS_SOCKET_ADDRESS`. `cal_force` and `cal_stress` are not forced: set either keyword explicitly in `inp` when that property must be computed on every frame. If omitted, the calculator starts with the properties requested by ASE and can restart the socket session when a later request adds forces or stress. An explicit `cal_force=0` or `cal_stress=0` is honored; requesting that disabled property raises `PropertyNotImplementedError` instead of returning zeros.
...
Calling `atoms.get_potential_energy()`, `atoms.get_forces()`, and `atoms.get_stress()` independently is supported. For an external optimizer, request energy and forces; for an i-PI NPT/barostat run, enable both `cal_force=1` and `cal_stress=1`. Energy-only and energy-plus-stress calculations are also supported for finite-difference or active-learning workflows. When SCF does not converge, the frame is still returned and `calc.last_scf_converged` records the status so the Python workflow can choose whether to accept or reject it.

A socket calculator owns one ABACUS process initialized from one fixed `INPUT`/`STRU` setup. Reuse the same instance for position updates under the same electronic-structure settings. `AbacusSocketIO(variable_cell=False)` is fixed-cell and rejects cell changes; use `AbacusSocketIO(variable_cell=True)` for complete 3x3 cell updates, including volume, anisotropic strain, and shear. Variable-cell mode requires a periodic rank-3 cell and leaves pressure control to the external driver or barostat. Do not change `kpts`, `kspacing`, `nspin`, `basis_type`, `basissets`, pseudopotentials, species, atom count, or other core `INPUT`/`STRU` parameters through an existing socket calculator; create a new calculator and client process for those changes.

In socket mode, ABACUS keeps one client process alive until the calculator is closed (a property expansion may intentionally start a new session). All SCF evaluations produced by a session are appended to the same `OUT.ABACUS/running_scf.log`, because the ABACUS calculation type remains `scf`. The authoritative per-step properties are returned through the i-PI socket to ASE. Use ASE trajectory and optimizer log files, such as `BFGS(atoms, trajectory="opt.traj", logfile="opt.log")`, when each optimizer or MD step should be saved separately. Treat `running_scf.log` mainly as the ABACUS diagnostic log for the socket client, not as one independent FileIO result per structure.
...
A complete fixed-cell validation and benchmark example is available in `interfaces/ASE_interface/examples/socketio.py`; the full-cell ASE-filter and official i-PI NPT/barostat validation is in `interfaces/ASE_interface/examples/socketio_variable_cell.py` and `interfaces/ASE_interface/examples/ipi_variable_cell/`.
